...
describe("sms handler", () => {
  ...
  it("should deny access if not a user or guest", async () => {
    ...
    expect(twiml.message).toHaveBeenCalledWith("No permissions to grant access. :(");
  });
  ...
  it("should deny guest other commands", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("Guests can only use the 'unlock' command. 🔒");
  });
  ...
  it("should handle 'status' command", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith(expect.stringContaining("users: {\"+12223334444\":\"Alice\"}"));
  });
  ...
  it("should handle 'lock' command", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith(expect.stringContaining("The door's locked"));
  });

  it("should handle 'addguest' command", async () => {
    const context = createMockContext(mockStatus);
    const event = { From: "+12223334444", Body: "addguest Dave +14155551234" } as any;
    const callback = createMockCallback();

    await handler(context, event, callback);

    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("Added guest Dave at +14155551234. 🐿");
  });
  ...
  it("should handle 'addguest' syntax error", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith(expect.stringContaining("Syntax: AddGuest"));
  });

  it("should handle 'adduser' command", async () => {
    const context = createMockContext(mockStatus);
    const event = { From: "+12223334444", Body: "adduser Dave +14155551234" } as any;
    const callback = createMockCallback();

    await handler(context, event, callback);

    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("Added Dave at +14155551234. 🦆");
  });
  ...
  it("should handle 'adduser' syntax error", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith(expect.stringContaining("Syntax: AddUser"));
  });
  ...
  it("should handle 'removeguest' when no guests exist", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("I don't know guest Charlie. 🐢");
  });
  ...
  it("should handle 'removeguest' not found", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("I don't know guest Unknown. 🐢");
  });
  ...
  it("should prevent self-removal", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("Can't remove yourself, dummy! 🐓");
  });
  ...
  it("should handle unknown command", async () => {
    ...
    const twiml = (callback.mock.calls[0][1] as any);
    expect(twiml.message).toHaveBeenCalledWith("Could not understand. 🐼 Try giving me a number.");
  });
